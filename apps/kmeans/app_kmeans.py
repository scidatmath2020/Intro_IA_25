...
# Subir archivo base
archivo = st.file_uploader("Sube tu archivo CSV para segmentación", type=["csv"])
...
if archivo is not None:
    ...
    if columnas_seleccionadas:
        ...
        if st.button("Ejecutar segmentación"):
            ...
            archivo_nuevo = st.file_uploader("Sube un nuevo archivo CSV con las mismas columnas", type=["csv"], key="nuevo")
            ...
            if archivo_nuevo is not None:
                ...
                if list(df_nuevo.columns) != columnas_seleccionadas:
                    ...
                    # Agregar nuevos puntos al gráfico existente
                    for i in range(n_clusters):
                        ...
                        ax.scatter(
                            ...
                            marker='x',
                            s=100,
                            label=f'Nuevos puntos Cluster {i+1}'
                        )
                    ax.set_title("Segmentación con nuevos datos")
                    ax.legend()
                    st.pyplot(fig)
...
4. Después, sube nuevos datos con las mismas columnas para predecir su clúster
""")
